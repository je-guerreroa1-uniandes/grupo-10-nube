...
resource "google_compute_instance" "api" {
  name         = "api"
  machine_type = "f1-micro" # [1]vCPU [614] MiB RAM
  zone         = var.zone
...
resource "google_compute_instance" "jobs" {
  name         = "jobs"
  machine_type = "f1-micro" # [1]vCPU [614] MiB RAM
  zone         = var.zone
  tags         = ["g10-vpc", "ssh", "https-server", "http-server"]
...
locals {
  authorizednets = [
    { name : "api", cidr : google_compute_instance.api.network_interface.0.access_config.0.nat_ip },
    { name : "jobs", cidr : google_compute_instance.jobs.network_interface.0.access_config.0.nat_ip }
  ]
}
